...
class MochaAdapter {
    constructor (cid, config, specs, capabilities) {
        ...
        this.retrySpec = this.config.mochaOpts.retrySpec || 0
        this.messageUIDs = {
            suite: {},
            hook: {},
            test: {}
    ...
    async run () {
        ...
        let result = await this.mochaRun(mocha)
    ...
    async mochaRun (mocha) {
        ...
        if (this.runner.failures && this.retryNeeded(mocha)) {
    ...
    retryNeeded (mocha) {
        const title = mocha.suite.suites[0].title
        this.retry[title] = this.retry[title] ? ++this.retry[title] : 1
        return this.retry[title] < this.retrySpec
    }
    ...
    // if we have an error and we can still retry
    // abort the current runner and put failing test to pending
    abortNeeded (err, payload) {
        return err && payload.parent && (!this.retry[payload.parent.title] || this.retry[payload.parent.title] < (this.retrySpec - 1))
    }
    ...
    emit (event, payload, err) {
        ...
        if (event.match('suite:') && this.retry[message.title]) {
            message.title += ` (retry ${this.retry[message.title]})`
        }

        if (event.match('test:') && payload.parent && this.retry[payload.parent.title]) {
            message.parent += ` (retry ${this.retry[payload.parent.title]})`
        }
